...
async function packagedMissingInventorySmokeTest() {
  ...
  try {
    ...
    const accessProbe = [
      ...
      "  if (![page, client, modelResponse, styles].every((response) => response.status === 200) || !pageText.includes('id=\"inventory-list\"') || !pageText.includes('id=\"analysis-filter\"') || !pageText.includes('id=\"best-confidence-filter\"') || !pageText.includes('id=\"required-coverage-filter\"') || !pageText.includes('id=\"provider-evidence-filter\"') || !pageText.includes('id=\"release-search-input\"') || !pageText.includes('id=\"release-protocol-filter\"') || !pageText.includes('id=\"release-shortlist\"') || !pageText.includes('id=\"release-decision-filter\"') || !pageText.includes('id=\"release-confidence-filter\"') || !pageText.includes('id=\"release-required-fit-filter\"') || !pageText.includes('id=\"release-language-filter\"') || !pageText.includes('id=\"release-language-confidence-filter\"') || !pageText.includes('id=\"release-leading\"') || !pageText.includes('id=\"release-sort-order\"') || !clientText.includes('credentials: \"omit\"') || !clientText.includes('?refresh=1') || !clientText.includes('selectReleases') || !clientText.includes('shortlistedReleases') || !clientText.includes('leadingRelease') || !clientText.includes('analysisByItem') || !clientText.includes('providerEvidenceLabel') || !stylesText.includes('@media (max-width: 760px)') || !stylesText.includes('.release-controls') || !stylesText.includes('.release-shortlist-card') || !stylesText.includes('.release-leading') || !stylesText.includes('.item-analysis-badge') || !stylesText.includes('provider-partial') || !page.headers.get('content-security-policy')?.includes(\"default-src 'self'\")) throw new Error('dashboard asset contract mismatch');",
