...
async function packagedMissingInventorySmokeTest() {
  ...
  const richReleaseScenario = "PEG-DOCKER-019";
  ...
  try {
    ...
    const accessProbe = [
      ...
      "  if (![page, client, modelResponse, styles].every((response) => response.status === 200) || !pageText.includes('id=\"inventory-list\"') || !pageText.includes('id=\"analysis-filter\"') || !pageText.includes('id=\"best-confidence-filter\"') || !pageText.includes('id=\"required-coverage-filter\"') || !pageText.includes('id=\"provider-evidence-filter\"') || !pageText.includes('id=\"application-filter\"') || !pageText.includes('id=\"profile-filter\"') || !pageText.includes('id=\"policy-language-filter\"') || !pageText.includes('id=\"analysis-age-filter\"') || !pageText.includes('id=\"clear-inventory-filters\"') || !pageText.includes('id=\"release-search-input\"') || !pageText.includes('id=\"release-protocol-filter\"') || !pageText.includes('id=\"release-shortlist\"') || !pageText.includes('id=\"release-shortlist-items\"') || !pageText.includes('id=\"release-decision-filter\"') || !pageText.includes('id=\"release-confidence-filter\"') || !pageText.includes('id=\"release-required-fit-filter\"') || !pageText.includes('id=\"release-language-filter\"') || !pageText.includes('id=\"release-language-confidence-filter\"') || !pageText.includes('id=\"release-leading\"') || !pageText.includes('id=\"release-sort-order\"') || !clientText.includes('credentials: \"omit\"') || !clientText.includes('?refresh=1') || !clientText.includes('selectReleases') || !clientText.includes('releaseComparison') || !clientText.includes('comparisonTable') || !clientText.includes('showReleaseInTable') || !clientText.includes('leadingRelease') || !clientText.includes('activeInventoryFilterCount') || !clientText.includes('populateInventoryAnalysisFilters') || !clientText.includes('analysisByItem') || !clientText.includes('providerEvidenceLabel') || !stylesText.includes('@media (max-width: 760px)') || !stylesText.includes('.inventory-filter-state') || !stylesText.includes('.release-controls') || !stylesText.includes('.release-comparison') || !stylesText.includes('.comparison-cell--stronger') || !stylesText.includes('.release-leading') || !stylesText.includes('.item-analysis-badge') || !stylesText.includes('provider-partial') || !page.headers.get('content-security-policy')?.includes(\"default-src 'self'\")) throw new Error('dashboard asset contract mismatch');",
      ...
      "  const acceptedReleases = dashboardModel.selectReleases(itemView.releases, { decision: 'accepted', sort: 'custom-format-desc' });",
      "  const rejectedReleases = dashboardModel.selectReleases(itemView.releases, { decision: 'rejected' });",
      "  const itemSummary = dashboardModel.itemAnalysisSummary(itemView);",
      "  const analyzedRows = dashboardModel.rowsWithAnalysis(rows, new Map([[rows[0].key, itemSummary]]));",
      ...
      "  const sonarrItems = dashboardModel.selectRows(analyzedRows, { application: 'sonarr' });",
      "  const matchingProfile = dashboardModel.selectRows(analyzedRows, { profile: 'profile:Synthetic policy' });",
      "  const matchingLanguage = dashboardModel.selectRows(analyzedRows, { language: 'language:pt-BR' });",
      "  const recentAnalysis = dashboardModel.selectRows(analyzedRows, { analysisAge: 'recent', nowEpochMs: Date.parse(itemSummary.generatedAt) + 1000 });",
      "  const triageFilterCount = dashboardModel.activeInventoryFilterCount({ application: 'sonarr', profile: 'profile:Synthetic policy', language: 'language:pt-BR', analysisAge: 'recent', query: 'synthetic' });",
      "  const richReleaseRows = dashboardModel.selectReleases(itemView.releases, { query: 'subtitle preference', protocol: 'torrent', sort: 'seeders-desc' });",
      "  const subtitleDecisionRows = dashboardModel.selectReleases(itemView.releases, { requiredFit: 'strong', language: 'pt-br', languageConfidence: 'confirmed' });",
      "  const leading = dashboardModel.leadingRelease(itemView.releases);",
      "  const shortlistRows = dashboardModel.shortlistedReleases(itemView.releases, [itemView.releases[0].id, 'missing-release']);",
      "  const comparisonRelease = { ...itemView.releases[0], id: 'comparison-release', title: 'Synthetic comparison release', confidence: 'likely', requiredFit: 'possible', customFormatScore: 0, seeders: 1, ageHours: 10, languages: itemView.releases[0].languages.map((language) => ({ ...language, confidence: 'likely' })) };",
    ...
    process.stdout.write(`${richReleaseScenario} packaged release exploration=local (rich metadata, search, protocol, sorts, shortlist, zero extra upstream requests)\n`);
