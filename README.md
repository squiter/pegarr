...
The API-feasibility milestone, Phase 1 read-only MVP, and Phase 2 controlled Grab are complete. Phase 3 has started with instance-safe identity across inventory, analysis, dashboard routes, Grab confirmation, duplicate protection, and audit history; multiple-instance configuration and runtime fan-out remain in progress. The implementation is synthetic-test proven; live Sonarr/Radarr mutation behavior and NAS compatibility remain explicit manual gaps. See the [controlled Grab guide](docs/controlled-grab.md), [Phase 1 completion record](docs/phase-1-completion.md), [Phase 2 completion record](docs/phase-2-completion.md), [active Phase 3 record](docs/phase-3.md), and [research proposal](ARR-SUBTITLE-RELEASE-PICKER-RESEARCH.md).
