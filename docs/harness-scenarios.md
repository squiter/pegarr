...
| PEG-MANUAL-003 | NAS runtime smoke test | Corrected immutable digest, container hardening, HTTPS readiness, persisted authentication/settings, mode-0600 password storage, successful SubDL evidence, enabled add-form loading, and six unaffected sibling services are live-proven; installed Bazarr assignments, remaining provider-secret permissions, and cache reuse remain open |
