...
| PEG-RELEASE-002 | Release procedure binds tags to exact-commit gates, public image verification, and explicit manual boundaries | `scripts/harness/architecture-contract.mjs` |
...
| PEG-DOCKER-027 | Packaged login survives restart, renews bounded expiry, and keeps logout durable | `scripts/harness/docker-build.mjs` |
...
| PEG-SESSION-004 | Hashed browser sessions survive a safe process restart without implicit renewal | `src/session-store.test.ts` |
| PEG-SESSION-005 | Authenticated browser activity renews a bounded inactivity expiry | `src/session-store.test.ts`, `src/app.test.ts` |
