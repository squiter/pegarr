...
- Completed locally: username/password authentication with bounded restorable server sessions, CSRF-bound settings/add mutations, explicit logout and expiry, legacy read-only bearer compatibility, secret-safe first-run prerequisite and capability guidance in an automatically opened but collapsible **Setup & settings** drawer, catalog-first navigation after setup, sanitized multi-instance catalog lookup, discovery-first dashboard, persistent per-language policy controls for required, forced, and hearing-impaired preferences, secret-safe SubDL/OpenSubtitles credential and mapping entry with a policy-derived fill helper, immediate UI-configured provider use, title-level pre-add coverage preview, opt-in explicit add-to-Arr with server-owned defaults and automatic search forced off, created-record identity verification, automatic Radarr exact analysis, Sonarr season/episode/specials scope selection into exact analysis, and administrator-confirmed controlled Grab from exact movie, episode, or season-pack continuation rows.
- Next: resume the release-candidate acceptance roadmap with operator-controlled live verification; local implementation of the discovery-first P0 path is complete.
- Installed Sonarr and Radarr add-to-exact-analysis compatibility and automatic-search suppression are verified. An operator-owned controlled Grab remains an explicit manual boundary. No continuation can silently Grab.
