...
- Completed locally: username/password authentication with bounded restorable server sessions, CSRF-bound settings/add mutations, explicit logout and expiry, legacy read-only bearer compatibility, sanitized multi-instance catalog lookup, discovery-first dashboard, persistent explicit default language policy, secret-safe SubDL/OpenSubtitles credential and mapping entry, immediate UI-configured provider use, title-level pre-add coverage preview, opt-in explicit add-to-Arr with server-owned defaults and automatic search forced off, created-record identity verification, automatic Radarr exact analysis, Sonarr season/episode scope selection into exact analysis, and administrator-confirmed controlled Grab from exact movie or episode continuation rows.
- Next: add first-run onboarding and clearer role/capability guidance without moving Arr or provider secrets into browser storage.
- Not yet implemented: automatic-search suppression verification against live Arr, richer forced/hearing-impaired policy controls, first-run onboarding, and season-pack controlled Grab. No continuation can silently Grab.
