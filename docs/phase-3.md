...
- `P3-OPENSUBTITLES` is complete at the provider-adapter boundary. Exact movie and episode searches use the official REST search route, API key and required application identity stay in headers, responses are bounded, download handles are discarded, and failures remain distinct from a successful empty result. Runtime provider scheduling is intentionally owned by `P3-PROVIDER-QUOTA`; successful installed credential, pt-BR mapping, quota, and cache-hit compatibility were recorded during the `v0.1.3` acceptance.
