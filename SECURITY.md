...
The browser UI uses a username/password login backed by a bounded server-side session in an `HttpOnly`, `SameSite=Strict` cookie. The password stays in a server-side secret file and is never returned to or stored by the browser. To survive safe process and container restarts, Pegarr stores only SHA-256 session and CSRF digests and their fixed expiry in the private mode-`0600` data volume; raw browser tokens are never written there, and logout deletes the durable entry. Legacy API clients may use the separate bearer access token; treat it as a credential, send it only in the `Authorization` header over HTTPS or a trusted private network, and never place it in browser storage, URLs, logs, issue attachments, or `.env` values.
